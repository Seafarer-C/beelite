...
import DatabaseViewerPage from "./pages/DatabaseViewerPage.vue";
...
const databaseViewerMode = ref(false);
...
function syncDevTestPageFromHash(): void {
  ...
  databaseViewerMode.value = h === "db-viewer";
}
...
function openDatabaseViewer(): void {
...
<template>
  ...
  <DatabaseViewerPage v-else-if="databaseViewerMode" @exit="exitDevTestPage" />
      ...
      <header class="workspace-topbar">
        ...
        <div class="topbar-actions">
          ...
          <button
            ...
            type="button"
            aria-label="打开数据库查看器"
            title="SQLite 数据库查看器（hash #db-viewer）"
            @click="openDatabaseViewer"
          >
            <Database :size="18" />
          </button>
          <button
